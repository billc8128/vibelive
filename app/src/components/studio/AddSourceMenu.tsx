...
// ────────────────────────────────────────────────────────────────
// AddSourceMenu — 一个 [+ 添加源] 按钮, 点开后显示 source 类型,
// 选一个就 onAdd(source). 不实现弹窗 / 模态, 用 inline 折叠.
//
// Live2D 选项进入二级菜单, 列出 LIVE2D_MODEL_REGISTRY 里的所有
// 预置模型 + 当前用户从 /api/vtuber-models 上传的私有模型, 点击
// 都直接当 Live2D source 加进 scene. 末尾的"上传新模型"按钮进入
// ModelUploader (文件夹直传 → 自动注册).
//
...
type Step = "closed" | "primary" | "live2d" | "live2d-upload";
          ...
          <div className="divide-y divide-border-pixel/40">
            ...
            {!userModelsLoading && userModels.length === 0 && !userModelsError && (
              <p className="px-3 py-2 text-[10px] text-text-secondary/60">
                还没有上传过模型 — 点底部 [+ 上传新模型] 添加 VTube Studio 文件夹
              </p>
            )}
            {userModels.map((m) => (
              <div
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                <p className="text-xs text-accent-cyan">上传新模型</p>
                <p className="text-[10px] text-text-secondary/70 truncate">
                  选 VTube Studio 文件夹 · 自动识别 model3.json
                </p>
              </div>
            </button>
          </div>
